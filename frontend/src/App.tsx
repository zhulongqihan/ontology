    ...
    <div className="admin-shell">
      <aside className="sidebar">
        ...
        <div className="engine-switcher" aria-label="当前引擎">
          ...
          <div><strong>{overview?.engine.name ?? '柔性引擎与本体化平台'}</strong><span>v{overview?.engine.version ?? '0.3.0'} · local</span></div>
        ...
        <div className="sidebar-footer"><span className="source-kicker">ENGINE ID</span><code>{overview?.engine.id ?? 'flexible-engine-ontology'}</code><span className="sidebar-note">本地持久化 · API 在线</span></div>
      </aside>
      ...
      <main className="main-area">
        ...
        <div className="content-wrap">
          ...
          {activeView === 'models' && <ModelsView models={models} ontologyTypes={ontologyTypes} selectedModel={selectedModel} onSelect={setSelectedModelId} onNavigate={setActiveView} onAdd={(payload) => afterMutation(() => engineApi.addModel(payload), '模型已写入引擎注册表')} onBind={(modelId, ontologyTypeId) => afterMutation(() => engineApi.updateModelOntologyBinding(modelId, ontologyTypeId), '模型本体绑定已更新')} />}
...
function ModelsView({ models, ontologyTypes, selectedModel, onSelect, onNavigate, onAdd, onBind }: { models: EngineModel[]; ontologyTypes: OntologyType[]; selectedModel?: EngineModel; onSelect: (id: string) => void; onNavigate: (view: ViewKey) => void; onAdd: (payload: { id: string; name: string; description: string; initialState: string }) => Promise<void>; onBind: (modelId: string, ontologyTypeId: string | null) => Promise<void> }) {
  ...
  return <><PageIntro eyebrow="MODEL REGISTRY / FLEXIBLE OBJECTS" title="模型管理" description="模型是柔性引擎的运行单元。选择模型后，可以继续管理它的字段版本、状态流程和运行输入。" action={<span className="identity-badge live">ENGINE STATE</span>} /><div className="split-layout"><div className="registry-stack"><section className="panel registry-panel"><PanelHeading kicker="REGISTERED MODELS" title={`${models.length} 个模型`} /><div className="registry-list">{models.map((model) => <button type="button" className={`registry-row ${selectedModel?.id === model.id ? 'selected' : ''}`} key={model.id} onClick={() => onSelect(model.id)}><span className="model-type">M</span><span className="model-copy"><strong>{model.name}</strong><small>{model.id} · 本体 {model.ontologyTypeId ?? '未绑定'}</small></span><span className="status-dot success" /></button>)}</div></section><section className="panel form-panel compact-form"><PanelHeading kicker="CREATE MODEL" title="新增运行模型" /><form onSubmit={submit} className="admin-form"><label>模型 ID<input value={id} onChange={(event) => setId(event.target.value)} placeholder="assessment-session" /></label><label>模型名称<input value={name} onChange={(event) => setName(event.target.value)} placeholder="评估会话" /></label><label>说明<input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="模型用途" /></label><label>初始状态<input value={initialState} onChange={(event) => setInitialState(event.target.value)} placeholder="DRAFT" /></label><button type="submit" className="primary-button">创建模型</button></form></section></div>{selectedModel ? <ModelDetail model={selectedModel} ontologyTypes={ontologyTypes} onBind={onBind} onNavigate={onNavigate} /> : <EmptyState title="没有模型" description="请先创建模型定义。" />}</div></>
}

function ModelDetail({ model, ontologyTypes, onBind, onNavigate }: { model: EngineModel; ontologyTypes: OntologyType[]; onBind: (modelId: string, ontologyTypeId: string | null) => Promise<void>; onNavigate: (view: ViewKey) => void }) {
  const [binding, setBinding] = useState(model.ontologyTypeId ?? '')
  useEffect(() => setBinding(model.ontologyTypeId ?? ''), [model.id, model.ontologyTypeId])
  async function submitBinding(event: FormEvent) { event.preventDefault(); await onBind(model.id, binding || null) }
  return <section className="panel model-detail"><div className="detail-hero"><div><span className="eyebrow">MODEL / {model.id}</span><h2>{model.name}</h2><p>{model.description}</p></div><StatusPill status="READY" /></div><div className="detail-facts"><Fact label="schema version" value={`v${model.schemaVersion}`} /><Fact label="initial state" value={model.initialState} /><Fact label="fields" value={String(model.fields.length)} /><Fact label="transitions" value={String(model.transitions.length)} /><Fact label="ontology binding" value={model.ontologyTypeId ?? 'UNBOUND'} /></div><div className="binding-editor"><form onSubmit={submitBinding}><label>运行时本体根类型<select aria-label="模型本体绑定" value={binding} onChange={(event) => setBinding(event.target.value)}><option value="">不绑定</option>{ontologyTypes.map((type) => <option value={type.id} key={type.id}>{type.label} · {type.id}</option>)}</select></label><button type="submit" className="quiet-button">保存绑定</button></form></div><div className="section-divider" /><PanelHeading kicker="MODEL CONTROL" title="进入管理模块" /><div className="quick-actions"><button type="button" onClick={() => onNavigate('schema')}><strong>Schema / 字段</strong><span>增加动态属性和版本规则 →</span></button><button type="button" onClick={() => onNavigate('workflow')}><strong>工作流</strong><span>配置状态和事件转换 →</span></button><button type="button" onClick={() => onNavigate('runtime')}><strong>运行调试</strong><span>输入数据并执行引擎 →</span></button></div><div className="detail-foot">最后更新：{formatTime(model.updatedAt)}<code>{model.id}</code></div></section>
}
...
function EvidenceView({ runs, models, auditEvents, idempotencyRecords, onRetry, onReplay, onRollback, onExport }: { runs: RuntimeRun[]; models: EngineModel[]; auditEvents: AuditEvent[]; idempotencyRecords: IdempotencyRecord[]; onRetry: (runId: string) => Promise<void>; onReplay: (runId: string) => Promise<void>; onRollback: (runId: string) => Promise<void>; onExport: () => Promise<void> }) {
  ...
  return <>
    ...
    <div className="evidence-layout">
      <section className="panel evidence-run-panel"><PanelHeading kicker="RUN JOURNAL" title={`${runs.length} 条运行记录`} />{runs.length === 0 ? <EmptyState title="暂无运行证据" description="先到运行调试提交一次真实执行。" /> : <div className="evidence-run-list">{runs.map((run) => <button type="button" className={`evidence-run-row ${selectedRun?.id === run.id ? 'selected' : ''}`} key={run.id} onClick={() => setSelectedRunId(run.id)}><StatusPill status={run.status} /><span><strong>{run.id}</strong><small>{run.modelId} · {run.contextId}{run.replayOfRunId ? ` · replay of ${run.replayOfRunId}` : ''}</small></span><code>v{run.schemaVersion ?? '—'} / w{run.workflowVersion ?? '—'}</code></button>)}</div>}</section>
      <section className="panel evidence-detail-panel"><PanelHeading kicker="RUN EVIDENCE" title={selectedRun ? selectedRun.id : '选择一次运行'} action={selectedRun && <div className="evidence-actions"><button type="button" className="quiet-button" onClick={() => void onReplay(selectedRun.id)}>重放</button><button type="button" className="quiet-button" disabled={selectedRun.status !== 'FAILED'} onClick={() => void onRetry(selectedRun.id)}>重试</button><button type="button" className="quiet-button" disabled={selectedRun.status !== 'PASSED'} onClick={() => void onRollback(selectedRun.id)}>回滚</button></div>} />{!selectedRun ? <EmptyState title="选择运行记录" description="运行、快照和 Trace 会在这里展开。" /> : <div className="evidence-detail">{evidenceError && <div className="error-list">! {evidenceError}</div>}<div className="evidence-facts"><Fact label="data identity" value={selectedRun.dataIdentity} /><Fact label="engine / schema" value={`v${selectedRun.engineVersion ?? '—'} / v${selectedRun.schemaVersion ?? '—'}`} /><Fact label="workflow" value={`v${selectedRun.workflowVersion ?? '—'}`} /><Fact label="ontology binding" value={selectedRun.ontologyTypeId ? `${selectedRun.ontologyTypeId} · v${selectedRun.ontologyVersion ?? '—'}` : 'UNBOUND'} /><Fact label="ontology definition" value={shortHash(selectedRun.ontologyDefinitionSha256)} /><Fact label="revision" value={String(selectedRun.contextRevision ?? '—')} /></div><div className="evidence-section"><div className="subheading"><strong>Snapshots</strong><span>{loadingEvidence ? '读取中…' : `${snapshots.length} 个封存快照`}</span></div><div className="snapshot-grid">{snapshots.map((snapshot) => <div className="snapshot-card" key={snapshot.phase}><div><StatusPill status={snapshot.phase} /><code>{shortHash(snapshot.sha256)}</code></div><strong>{snapshot.state}</strong><small>{snapshot.status} · {formatTime(snapshot.capturedAt)}</small><pre>{JSON.stringify(snapshot.values, null, 2)}</pre></div>)}</div></div><div className="evidence-section"><div className="subheading"><strong>Trace spans</strong><span>{trace?.lifecycle ?? 'LEGACY_UNKNOWN'} · {trace?.sealed ? 'SEALED' : '—'} · {formatDuration(trace?.durationNs ?? selectedRun.durationNs, trace?.durationMs ?? selectedRun.durationMs)}</span></div><div className="span-list">{(trace?.spans ?? []).map((span) => <div className="span-row" key={span.spanId}><code>{span.name}</code><StatusPill status={span.status} /><span>{formatDuration(span.durationNs, span.durationMs)}</span></div>)}</div></div></div>}</section>
    </div>
    <div className="evidence-grid"><section className="panel"><PanelHeading kicker="VERSION BINDINGS" title="模型版本" /><div className="version-list">{models.map((model) => <div className="version-row" key={model.id}><span><strong>{model.name}</strong><small>{model.id}</small></span><code>Schema v{model.schemaVersion} · Workflow v{model.workflowVersion}</code></div>)}</div></section><section className="panel"><PanelHeading kicker="AUDIT JOURNAL" title={`${auditEvents.length} 条审计事件`} /><div className="audit-list">{auditEvents.slice(0, 8).map((event) => <div className="audit-row" key={event.id}><StatusPill status="READY" /><span><strong>{event.action}</strong><small>{event.targetType} · {event.targetId} · r{event.beforeRevision}→r{event.afterRevision} · {event.changes.length} 项变更</small></span><code>{formatTime(event.createdAt)}</code></div>)}</div>{auditEvents.length === 0 && <EmptyState title="暂无审计事件" description="配置和回滚动作会追加审计记录。" />}</section><section className="panel"><PanelHeading kicker="IDEMPOTENCY" title={`${idempotencyRecords.length} 条幂等记录`} /><div className="audit-list">{idempotencyRecords.slice(0, 8).map((record) => <div className="audit-row" key={`${record.scope}-${record.key}`}><span><strong>{record.key}</strong><small>{record.scope} · {record.runId}</small></span><code>{shortHash(record.requestSha256)}</code></div>)}</div>{idempotencyRecords.length === 0 && <EmptyState title="暂无幂等键" description="带幂等键的运行请求会在这里留下绑定。" />}</section></div>
  </>
}
...
function shortHash(value?: string | null) { if (!value) return '—'; return value.length > 12 ? `${value.slice(0, 8)}…${value.slice(-4)}` : value }
function formatDuration(durationNs?: number, durationMs?: number) { const nanos = durationNs && durationNs > 0 ? durationNs : Math.max(0, durationMs ?? 0) * 1000000; if (nanos <= 0) return 'N/A'; if (nanos < 1000) return `${nanos} ns`; if (nanos < 1000000) return `${(nanos / 1000).toFixed(1)} μs`; return `${(nanos / 1000000).toFixed(2)} ms` }
